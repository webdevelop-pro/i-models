...
func (model Transfer) Save(ctx context.Context) error {
	if model.ID == 0 {
		err := errors.Errorf("%s: Transfer %d", orm.ErrEmptyID, model.ID)
	...
	updated, err := orm.Update[Transfer](
		ctx,
		model.db,
		map[string]any{
			"id": model.ID,
	...
	if updated == false {
		err := errors.Errorf("%s: Transfer %d", orm.ErrNoRowsAffected, model.ID)
